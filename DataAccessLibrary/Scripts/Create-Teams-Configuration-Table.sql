-- =============================================
-- Create TeamsBotConfigurations Table
...
BEGIN
    ...
    PRINT 'TeamsBotConfigurations table created successfully.';
END
ELSE
BEGIN
    PRINT 'TeamsBotConfigurations table already exists.';
END
GO
...
CREATE PROCEDURE [dbo].[usp_InsertTeamsBotConfiguration]
    ...
    @AppId NVARCHAR(100),
    @AppPassword NVARCHAR(500), -- Already encrypted by application
...
BEGIN
    ...
    INSERT INTO [dbo].[TeamsBotConfigurations] (
        [Id], [AppId], [AppPassword], [MessagingEndpoint],
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_GetActiveTeamsBotConfiguration]') AND type in (N'P', N'PC'))
    DROP PROCEDURE [dbo].[usp_GetActiveTeamsBotConfiguration];
GO
...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[usp_UpdateTeamsBotTestResult]') AND type in (N'P', N'PC'))
    DROP PROCEDURE [dbo].[usp_UpdateTeamsBotTestResult];
GO
...
PRINT 'Teams bot configuration schema created successfully.';
PRINT 'Use usp_InsertTeamsBotConfiguration to add configurations.';
PRINT 'Use usp_GetActiveTeamsBotConfiguration to retrieve the active bot.';
PRINT 'Use usp_UpdateTeamsBotTestResult to update test results.';
