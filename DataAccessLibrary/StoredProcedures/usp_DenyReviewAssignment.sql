...
-- Description: Deny a review assignment or access request with full audit trail
--              Handles both ReviewAssignment and AccessRequest types
--              Performance target: <500ms with audit
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[usp_DenyReviewAssignment]
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            VALUES (
                ...
                @PreviousDecision,
                @Justification,
                ISNULL(@Comments, 'Denied by ' + @DecisionMakerName),
                @DecisionMakerId,
                @DecisionMakerName,
                @DecisionMakerEmail,
                @NowUtc,
            ...
            -- Create remediation action for denied access (revoke)
            -- TODO: RemediationActions table schema needs to be created/updated
            -- Commenting out for now until remediation action infrastructure is ready
            /*
            DECLARE @ReviewTargetId UNIQUEIDENTIFIER;
            DECLARE @ReviewTargetType NVARCHAR(50);
